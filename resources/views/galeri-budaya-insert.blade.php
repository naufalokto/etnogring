...
<head>
    ...
    <style>
        ...
        .form-textarea { height: 110px; font-size: 16px; padding: 10px; }
        ...
        <main class="main-content">
            ...
            <div class="form-section">
                <div class="container">
                    ...
                    <div class="form-required-note">Field wajib diisi: Judul, Jenis, Isi Ringkas Kegiatan, dan (opsional) Tanggal Kegiatan serta Gambar untuk tampilan lebih baik.</div>
                ...
                <form action="<?php echo e(route('galeri-budaya.store')); ?>" method="POST" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                        ...
                        <div class="form-group">
                            <label for="isi_kegiatan" class="form-label required">Isi Ringkas Kegiatan</label>
                            <textarea name="isi_kegiatan" id="isi_kegiatan" class="form-textarea <?php echo e($errors->has('isi_kegiatan') ? 'is-invalid' : ''); ?>" placeholder="Tuliskan artikel selengkapnya disini !" required><?php echo e(old('isi_kegiatan')); ?></textarea>
                        <?php if($errors->has('isi_kegiatan')): ?>
                            <div class="field-error"><?php echo e($errors->first('isi_kegiatan')); ?></div>
                        <?php endif; ?>
                    </div>
            ...
            <div class="content-management-section">
                <div class="container">
                    <h2 class="content-management-title">Kelola Konten Galeri Budaya</h2>
                    <div class="content-management-note">Hapus konten yang tidak diperlukan atau sudah tidak relevan.</div>
                    
                    <div class="content-list">
                        @php
                            $galeriList = \App\Models\GaleriBudaya::orderBy('created_at', 'desc')->get();
                        @endphp
                            ...
                            @foreach($galeriList as $item)
                                <div class="content-item">
                                    <div class="content-image">
                                        <img src="{{ asset('images/gallery/' . $item->foto) }}" alt="{{ $item->title }}" onerror="this.src='{{ asset('images/placeholder.jpg') }}'">
                                    </div>
                                    <div class="content-details">
                                        ...
                                        @if($item->tanggal_kegiatan)
                                            <p class="content-date">Tanggal Kegiatan: {{ \Carbon\Carbon::parse($item->tanggal_kegiatan)->format('d-m-Y') }}</p>
                                        @endif
                                        <p class="content-description">{{ Str::limit($item->isi_kegiatan, 100) }}</p>
                                    </div>
                                    <div class="content-actions">
                                        <form action="{{ route('galeri-budaya.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus konten ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @else
                            <div class="no-content">
                                <p>Belum ada konten galeri budaya.</p>
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <script>
        ...
        (function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                const isi = form.querySelector('[name="isi_kegiatan"]');
                const foto = form.querySelector('[name="foto"]');

                [judul, jenis, isi, foto].forEach((field) => { if (field) field.style.borderColor = ''; });
                ...
                if (!isi.value.trim())   { valid = false; firstInvalid = firstInvalid || isi;   showError(isi, 'Wajib diisi'); }
